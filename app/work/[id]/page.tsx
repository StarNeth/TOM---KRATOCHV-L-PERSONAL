...
export default function ProjectDetail() {
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    if (isTransitioning.current || isFullscreen || !project) return;
  ...
  }, [isFullscreen, slug, router, project?.zone]);
  ...
  return (
    <main ref={containerRef} className="relative w-full h-[100svh] overflow-hidden bg-transparent text-white selection:bg-white selection:text-black">
      
      <div className="transition-curtain fixed inset-0 z-[9999] bg-[#020203] pointer-events-none" />
      <WebGLScene />

      <nav className="absolute top-0 left-0 w-full p-6 md:p-10 z-[100] flex justify-between items-start pointer-events-none ui-element">
      ...
      <div className="w-full h-full flex flex-col justify-center px-6 md:px-12 lg:px-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 xl:gap-24 w-full max-w-[1920px] mx-auto items-center">
          
          <div className={`col-span-1 lg:col-span-5 flex flex-col gap-8 z-10 transition-all duration-700 ${isFullscreen ? 'opacity-0 translate-x-[-50px] pointer-events-none' : 'opacity-100 translate-x-0'}`}>
            
            {/* DEFINITIVNÍ FIX OŘEZU (Anti-Gilotina) */}
              ...
              {project.title.split(" ").map((word, i) => (
                <span key={i} className="overflow-hidden flex pb-4 pr-8">
                  {word.split("").map((char, j) => (
                    <span key={j} className="detail-title-char font-syne font-black text-[clamp(3.5rem,7vw,7rem)] uppercase tracking-tighter leading-[0.85] text-white inline-block pr-[0.3em] -mr-[0.3em]">
                      {char}
                    </span>
                  ))}
                </span>
          ...
          <div className="col-span-1 lg:col-span-7 relative h-[50vh] lg:h-[75vh] w-full z-[150] ui-element">
            <div 
            ...
            <div 
              ref={macWindowRef}
              className={`absolute top-0 left-0 w-full h-full overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.8)] bg-[#050505] border border-white/10 flex flex-col rounded-[2.5rem] ${isFullscreen ? 'fixed z-[999]' : ''}`}
            >
              <div className="w-full h-12 flex-shrink-0 border-b border-white/5 flex items-center justify-between px-6 bg-[#111111]/95 backdrop-blur-md group/mac relative z-10 cursor-default">
                <div className="flex gap-2.5">
                   <button onClick={(e) => { e.stopPropagation(); isFullscreen ? setIsFullscreen(false) : router.push('/#work') }} className="w-3.5 h-3.5 rounded-full bg-[#ff5f56] flex items-center justify-center transition-colors outline-none cursor-pointer">
                     <span className="opacity-0 group-hover/mac:opacity-100 text-[#4c0000] text-[8px] leading-none mb-[1px] font-bold">✕</span>
                   </button>
                   <button onClick={(e) => { e.stopPropagation(); setIsFullscreen(false) }} className="w-3.5 h-3.5 rounded-full bg-[#ffbd2e] flex items-center justify-center transition-colors outline-none cursor-pointer">
                     <span className="opacity-0 group-hover/mac:opacity-100 text-[#593e00] text-[8px] leading-none mb-[1px] font-bold">−</span>
                   </button>
                   <button onClick={(e) => { e.stopPropagation(); setIsFullscreen(!isFullscreen) }} className="w-3.5 h-3.5 rounded-full bg-[#27c93f] flex items-center justify-center transition-colors outline-none cursor-pointer">
                     <span className="opacity-0 group-hover/mac:opacity-100 text-[#004d00] text-[8px] leading-none mb-[1px] font-bold">⤢</span>
                   </button>
                </div>
                <div className="font-mono text-[9px] text-white/30 tracking-widest uppercase pointer-events-none">
              ...
              <div className="w-full h-full overflow-y-auto custom-scrollbar relative bg-[#020202] z-10" data-lenis-prevent="true" onWheel={(e) => e.stopPropagation()}>
                <img src={project.image} alt={`${project.title} Interface`} className="w-full h-auto block object-top" loading="lazy" />
              </div>
            </div>
          </div>
      ...
      <div className={`absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 transition-opacity duration-500 ${isFullscreen ? 'opacity-0' : 'opacity-100'} mix-blend-difference pointer-events-none ui-element`}>
